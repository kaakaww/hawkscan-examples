...
// Need to overload this function for
// assumption 1: tokens come back as JSON OR
// assumption 2: tokens come back as Set-Cookie: headers
// then we will comment out the one that doesn't apply.
...
// This function is called on each request to allow the request to be modified before it is sent to the web application.
...
fun getRequiredParamsNames(): Array<String> {
    return arrayOf("fg-token", "id-token", "refresh-token")
}
